...
class TestProductRoutes(TestCase):
    ...
    def test_update_product(self):
        ...
        test_product = self._create_products(1)[0].serialize()
        test_product['description'] = 'test update REST call'
        response = self.client.put(f"{BASE_URL}/{test_product['id']}", json=test_product)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.get_json()
        self.assertEqual(data['description'], 'test update REST call')

    def test_delete_product(self):
        """It should  update a Product"""
        test_products = self._create_products(5)
        product_count = self.get_product_count()
        delete_product = test_products[0]
        response = self.client.delete(f"{BASE_URL}/{delete_product.id}")
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(len(response.data), 0)
        response = self.client.get(f"{BASE_URL}/{delete_product.id}")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        new_count = self.get_product_count()
        self.assertEqual(new_count, product_count - 1)

    def test_list_all_products(self):
        """It should  list all Productds"""
        test_products = self._create_products(5)
    ...
    def test_get_products_by_category(self):
        ...
        count = len([product for product in test_products if product.category == test_category])
        response = self.client.get(
